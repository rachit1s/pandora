...
public abstract class AbstractUpdateRequestForm extends AbstractEditRequestForm implements IUpdateRequestForm {

	public AbstractUpdateRequestForm(UIContext parentContext) {
		super(parentContext);
	}
	...
	private void setBooleanDefaultValue(){
    	...
    	if(baClient != null){
    		...
    		if(sysConfigClient != null){
    			...
    			if(config != null){
			    	if(sysConfigClient.getActionNotify() == 1)
			    ...
			    if(config != null){
		    		if(sysConfigClient.getActionNotifyLoggers())
			    		config.setPOJO(new POJOBoolean(true));
			    	else
			    		config.setPOJO(new POJOBoolean(false));
    			}
