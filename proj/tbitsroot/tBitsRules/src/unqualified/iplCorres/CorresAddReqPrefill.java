...
public class CorresAddReqPrefill implements IExtUIRenderer 
{
...
//    		String location = userInfo.get(UserInfoManager.LOCATION ) ;
			...
			ArrayList<String> tagList, BusinessArea ba, int action )  throws TBitsException 
	{
		...
		if( IExtUIRenderer.ADD_REQUEST != action )
			return ;		
		...
		URL fileURL = getClass().getResource(IPLConstants.ADD_REQ_PREFILL_FILE) ;
		if( null == fileURL )
		{
			LOG.error( "File not found = " + IPLConstants.ADD_REQ_PREFILL_FILE ) ;
			String sher = IPLConstants.showError(IPLConstants.MSG_CANNOT_PREFILL) ;
			tagTable.put(IPLConstants.EXT_SUBMIT_BUTTON_LIST, sher ) ;
			return ;
		}
...
//		}	
		...
		String previewHtmlButton = IPLConstants.getExtSubmitButtonHTML("previewPdfButton", "previewPdfButton", "PreviewHTML", "javascript:onPreview('html')" ) ;
...
//		boolean isWPCLMember = login_firm.trim().equalsIgnoreCase( KskConstants.WPCL_FIRM_NAME ) ? true : false ;
		...
		try
		{
			...
			for( Iterator<Type> iter = tal.iterator() ; iter.hasNext() ; )
			{
				...
				{
					params.put("corGenAgencyIndex_value", Integer.toString(type.getOrdering() - 1) ) ; break ;
				}
			}		
