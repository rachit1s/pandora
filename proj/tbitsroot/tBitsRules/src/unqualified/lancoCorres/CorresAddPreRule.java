...
public class CorresAddPreRule implements IRule 
{
	...
	String getCorrNo(Connection con, Hashtable<String,String> logger0Info, Type corrType ) throws SQLException
	{		
		String corresNo = "IPRWL-" ;
		char firm_letter = logger0Info.get(UserInfoManager.FIRM).toUpperCase().charAt(0) ;
		firm_letter = firm_letter == 'W' ? 'L' : 'I' ;
		corresNo += firm_letter + "-" + logger0Info.get(UserInfoManager.LOCATION) + "-" ;		
...
//		String replacement = validDash+"" ;
	...
	/*
	 * formats : WCG-[SW]-[two digit location]
	 */	
	VerificationResult sanitizeAndValidateCorrNo(String iCorrNo, Hashtable<String,String> logger0Info, Type corrType )
	{
...
//				prefix = getPrefix() ;
			...
			case 3 : //
				...
				// format = [SWsw]-[0-9]?[0-9]-[0-9]* i.e. no corr. Type info
				// no verification done in this case ... just pass this value on after
				Pattern ex = Pattern.compile("[SW]-[0-9]?[0-9]-[0-9]*") ;
			...
			case 4 : // format = [SW]-loc-corr.type-num // [SW]-[0-9]?[0-9]-[A-Z][A-Z]-[0-9]*
				String temp4 = "WCG-" + tokens[0] + "-" + makeXDigit(tokens[1],2) + "-" + tokens[2] ;
...
//				Pattern p = Pattern.compile("[SWPM]-[0-9]?[0-9]-[A-Z][A-Z]-[0-9]?[0-9]-[0-9]+") ;
				String temp = "WCG-" + tokens[0] + "-" + makeXDigit(tokens[1],2) + "-" +tokens[2] ;
				// check year 
		...
		VerificationResult vr = verifyAndIncrement( prefix, num ) ; 
		if(vr.success)
		{
...
//				myYear = new Timestamp().toCustomFormat("yy") ;		
	...
	public VerificationResult verifyAndIncrement(String name, int value ) 
	{
		Connection con = null ;
...
//					con.rollback() ;
	...
	{	
		...
		if( !ba.getSystemPrefix().equalsIgnoreCase(KskConstants.CORR_SYSPREFIX) ) 
...
//			boolean isAddRequest ;
			...
			{									
				...
				myLoggerList = (String) myCurrRequest.myMapFieldToObjects.get(KskConstants.CORR_LOGGER_FIELD_NAME) ;
			...
			//	myLogger0Id = myLogger0User.getUserId() ;
				...
				myAssList = (String) myCurrRequest.myMapFieldToObjects.get(KskConstants.CORR_ASSIGNEE_FIELD_NAME ) ;
				...
				String corrProtName = (String) myCurrRequest.myMapFieldToObjects.get(KskConstants.CORR_CORRESPONDANCE_PROTOCOL_FIELD_NAME) ;		
				try {
				...
				String corrTypeName = (String) myCurrRequest.myMapFieldToObjects.get(KskConstants.CORR_TYPE_FIELD_NAME) ;
				try {
				...
				String corrInitName = (String) myCurrRequest.myMapFieldToObjects.get(KskConstants.CORR_CORRESPONDANCE_INITIATOR_FIELD_NAME) ;
				try {
				...
				String myLoginInfoFirm = myLoginInfo.get(UserInfoManager.FIRM ) ;
				...
				else if( ! myLogger0InfoFirm.equalsIgnoreCase(myLoginInfoFirm) ) // && !myLogger0Info.get(UserInfoManager.FIRM).trim().equalsIgnoreCase(KskConstants.SEPCO_FIRM_NAME) )
				{
					throw new TBitsException("You (" + myLoginUser.getUserLogin() + ") are not allowed to log on behalf of (" + myLogger0User.getUserLogin() + ")" ) ;
				}
				...
				mySubList = (String)myCurrRequest.myMapFieldToObjects.get(KskConstants.CORR_SUBSCRIBER_FIELD_NAME) ;
...
//						subSet.add(KskConstants.KSK_BOSS_LOGIN) ;		
					...
					{						
						...
						ArrayList<RequestUser> reqUser = currentRequest.getSubscribers() ;
				...
				if( null != iCorrNo )
				{
					...
					{
						VerificationResult vres = sanitizeAndValidateCorrNo(iCorrNo, myLogger0Info, myCorrType ) ;
						if( vres.success )			
						{
							// set the corr. no. to what was sent in the result
							// set the correspondance no. 
							RequestEx corrNoRE = extendedFields.get(Field.lookupBySystemIdAndFieldName(ba.getSystemId(), KskConstants.CORR_CORRESPONDANCE_NUMBER_FIELD)) ;
...
//							return new RuleResult( true, "Skipping the generation of correspondance as the correpondence number field was set and correct.", true ) ;
			...
			if(createCorresFile )
			{
				...
				String curratt = fileRequestex.getTextValue() ;
				...
				{		
					...
					String corresNo = getCorrNo(con, myLogger0Info, myCorrType ) ;
		...
		//			String dear = "Dear " + ( assignee_sex.trim().equalsIgnoreCase("M") ? "Sir," : "Madam," );
				...
				catch( Exception e ) 
				{
					...
					{	if( null != con )
						...
						LOG.warn("Exception while connection rollback. May lead to corrupted Correspondence Number.") ;
						e1.printStackTrace();
					}				
					throw e ;
				}
				finally
				{
					try {
						...
						{
							e.printStackTrace();
							LOG.warn("Connection to the database cannot be closed.") ;
						}			
				}
			}
			else // do not generate the correspondence file
			{
				Calendar ndd = Calendar.getInstance() ;
				if( isAddRequest )
				{
