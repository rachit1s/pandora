 ...
 *
 * ID: $Id: Bitap.java,v 1.2 2007-12-14 20:24:31 tok Exp $
 */
...
public class Bitap // implements Locator
{
    ...
    public Bitap(String text, String pattern)
    {
      this.text = text;
    ...
    public double locate(int loc)
    {
        ...
        if (bestLoc != -1)
        {
        	double s = bitapScore(0, bestLoc);
            scoreThreshold = Math.min(s, scoreThreshold);
            return s;
        }
    ...
    public static double match(String text, String pattern){
    	Bitap a = new Bitap(text.trim(),pattern.trim());
    	double l = a.locate(0);
    	return l;
    }
    ...
    public static void main(String[] args) {
    	...
    	String[] text = {"IMP DR. NO.","FMGDR.NO. I","ABC"};
    	String dr1 = "FMG DR. NO.";
		String dr2 = "IMP DR. NO.";
		List<String> result = new ArrayList<String>();
		result.add("IMP DR. NO.");
		result.add("FMGDR.NO. I");
		result.add("ABC");
		
//		for(int i=0;i<text.length;i++){
//	    	double t = Bitap.match(text[i],dr1);
//	    	System.out.println("match1:"+t);
//	    	t = Bitap.match(text[i],dr2);
//	    	
//	    	System.out.println("match2:"+t);
//	    	}
		
		int index1=-1,index2=-1;
		double minScore1 = 1.0, minScore2 = 1.0;
		for (String str : result) {
			double score = Bitap.match(str.replaceAll(" ",""),dr1.replaceAll(" ",""));
			System.out.println(str + "match" +dr1 +"="+score);
			if (score < minScore1) {
				minScore1 = score;
				index1 = result.indexOf(str);
			}
			
			score = Bitap.match(str.replaceAll(" ",""),dr2.replaceAll(" ",""));
			System.out.println(str + "match" +dr2 +"="+score);
			if (score < minScore2) {
				minScore2 = score;
