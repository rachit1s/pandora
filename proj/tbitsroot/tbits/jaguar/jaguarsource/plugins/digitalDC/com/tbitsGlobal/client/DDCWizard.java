...
public class DDCWizard extends TransmittalAbstractWizard {
	...
	public DDCWizard() {
		...
		DDCConstants.dbService.fetchConstants(ClientUtils.getCurrentBA()
			...
			public void onSuccess(HashMap<String, Object> arg0) {
				...
				DDCClientConstants.fields = tmd;

				for (String key : hm1.keySet()) {
					if (key.equals("deliverableFieldID")) {
						DDCClientConstants.setDeliverableFieldID(Integer
								.parseInt(hm1.get(key)));
					} else if (key.equals("TRN_PROCESS_ID")) {
						DDCClientConstants.setTRN_PROCESS_ID(Integer
								.parseInt(hm1.get(key)));

					} else if (key.equals("docNoField")) {
						DDCClientConstants.setDocField(hm1.get(key));

					}

					DDCWizardPage page1 = new DDCWizardPage(context, DDCWizard.this);
					DDCGridWizardPage page2 = new DDCGridWizardPage(context, DDCWizard.this,
							dataObject);
					DDCFinalGridWizardPage page3 = new DDCFinalGridWizardPage(context);
					DDCWizard.this.addPage(page1);
					DDCWizard.this.addPage(page2);
					DDCWizard.this.addPage(page3);
					DDCWizard.this.show();
