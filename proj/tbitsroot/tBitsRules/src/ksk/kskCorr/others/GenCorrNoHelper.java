...
public class GenCorrNoHelper 
{
	...
	{
		// return corresPrefixConst + "-" + firmLetter + "-" + co.getGenerationAgency().getDescription() + "-" + corrType.getName() + "-" + yr;
	...
	public static String getKskPrefix(CorrObject co) throws CorrException
	{
		String corresPrefixConst = "KMP" ;
		
		String firmLetter = "?" ;
//		User log = co.getUserMapUsers().get(0);
//		if( log.getLocation() == null || log.getLocation().trim().equals("") )
//			throw new CorrException("The location field is empty for user : " + log.getUserLogin());
			
		if( co.getGenerationAgency() == null || co.getGenerationAgency().getName().trim().equals(""))
			throw new CorrException("Generation Agency was not set.");
		
		firmLetter = co.getGenerationAgency().getName().toUpperCase().substring(0, 1);
		
		Field corrTypeField = null;
		try 
		{
			corrTypeField = Field.lookupBySystemIdAndFieldName(co.getBa().getSystemId(), KskConst.CorrTypeFieldName);
		} catch (DatabaseException e1) {
			e1.printStackTrace();
		}
		
		if( null == corrTypeField )
			throw new CorrException("Cannot find field with name : " + KskConst.CorrTypeFieldName);
		
		String corrTypeValue = co.getAsString(corrTypeField.getName());
		
		if( null == corrTypeValue )
			throw new CorrException("Cannot find value in field " + corrTypeField.getDisplayName());
		
		Type corrType = null;
		try {
			corrType = Type.lookupBySystemIdAndFieldIdAndTypeName(co.getBa().getSystemId(), corrTypeField.getFieldId(), corrTypeValue);
		} catch (DatabaseException e) {
			e.printStackTrace();
		}
		
		if(null == corrType )
			throw new CorrException("Illegal value in field " + corrTypeField.getDisplayName());
