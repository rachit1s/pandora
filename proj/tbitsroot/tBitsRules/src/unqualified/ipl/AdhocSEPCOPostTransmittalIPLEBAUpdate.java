 ...
 *
 */
public class AdhocSEPCOPostTransmittalIPLEBAUpdate implements ITransmittalRule {

	private static final String DECISION_FROM_IPLP = "DecisionFromIPLP";
	...
	public void process(Connection connection, int transmittalId, Request transmittalRequest,
		...
		if (IPLUtils.isAdhocBHELDCR(dcrBA) && currentBA.getSystemPrefix().trim().equals(IPLUtils.ADHOC_IPLE)){
			/*
			...
			if (transmittalType.equals("TransmitToIPLE")){
				...
				//3. Flow Status From BHEL is changed to Under Review
				paramTable.put(Field.STATUS, IPLUtils.STATUS_UNDER_REVIEW);
				//4. Decision to BHEL is changed to None
				paramTable.put(Field.SEVERITY, IPLUtils.STRING_NONE);
				//5.	Flow Status To TCE is changed to Pending Submission
				paramTable.put(IPLUtils.FLOW_STATUS_TO_DCPL, IPLUtils.PENDING_SUBMISSION);
				//6.	Decision from TCE is changed to None
