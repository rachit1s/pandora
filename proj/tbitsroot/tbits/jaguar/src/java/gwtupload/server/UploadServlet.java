...
public class UploadServlet extends HttpServlet implements Servlet {
  ...
  protected static TBitsLogger logger = TBitsLogger.getLogger(UploadServlet.class.getName());
...
//    super.init(config);
// removed maxSize as we don't need it.
    if (isAppEngine()) {
...
//      uploader.setSizeMax(maxSize);
      ...
      for (FileItem fileItem : uploadedItems) {
        if (fileItem.isFormField() || fileItem.getSize() > 0) {
          ...
          error += "\nError.\nPlease verify that the file " + fileItem.getName() + " exists and its size is more that 0 bytes and you have enough permissions to read it";
        }
      }
