...
public class LogisticsAddRequestForm extends AddRequestForm{
	...
	public LogisticsAddRequestForm(UIContext parentContext, Stage stage) {
		super(parentContext);
		...
		this.addButton(new Button(LogisticsUtils.getAddHeadingString(this.stage.getParams()), new SelectionListener<ButtonEvent>(){
			...
			public void componentSelected(ButtonEvent ce) {
				LogisticsComponentWindow window = new LogisticsComponentWindow(LogisticsAddRequestForm.this.stage);
				window.show();
	...
	public void afterSubmit(final String sysPrefix, final int requestId) {
		...
		LogisticsConstants.logisticsService.setPreStageRequests(stage, requestId, preRequestIds, new AsyncCallback<Boolean>(){
			...
			public void onSuccess(Boolean result) {
				if(result){
					LogisticsAddRequestForm.super.afterSubmit(sysPrefix, requestId);
				}else{
