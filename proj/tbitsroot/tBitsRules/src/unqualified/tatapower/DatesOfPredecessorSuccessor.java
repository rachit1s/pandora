 ...
 *
 */
public class DatesOfPredecessorSuccessor implements IRule {
	...
	public RuleResult execute(Connection connection, BusinessArea ba,
		...
		if (isApplicable && (!isAddRequest) && (!reqTypeId.getName().startsWith("GRN"))
			...
			try {				
				Hashtable<Integer, String> fieldHierarchyHT = TataPowerUtils.getFieldHierarchy();
				...
				int key = TataPowerUtils.DCC_INDEX;
				switch (key){
					case TataPowerUtils.DCC_INDEX : {						
						isPredecessorAckDateFieldSet = setPredecessorAckField(
								oldRequest, extendedFields, systemId,
								ruleResult, fieldHierarchyHT, 
								TataPowerUtils.DCC_INDEX);
						if (isPredecessorAckDateFieldSet)
							return ruleResult;
					}
					case TataPowerUtils.SITE_IC_INDEX :{
						isPredecessorAckDateFieldSet = setPredecessorAckField(
								oldRequest, extendedFields, systemId,
								ruleResult, fieldHierarchyHT, 
								TataPowerUtils.SITE_IC_INDEX);
						if (isPredecessorAckDateFieldSet)
							return ruleResult;
					}
					case TataPowerUtils.QS_INDEX :{
						isPredecessorAckDateFieldSet = setPredecessorAckField(
								oldRequest, extendedFields, systemId,
								ruleResult, fieldHierarchyHT, 
								TataPowerUtils.QS_INDEX);
						if (isPredecessorAckDateFieldSet)
							return ruleResult;
					}
					case TataPowerUtils.GH_INDEX :{
						isPredecessorAckDateFieldSet = setPredecessorAckField(
								oldRequest, extendedFields, systemId,
								ruleResult, fieldHierarchyHT, 
								TataPowerUtils.GH_INDEX);
						if (isPredecessorAckDateFieldSet)
							return ruleResult;
					}
					case TataPowerUtils.HOC_INDEX:{
						isPredecessorAckDateFieldSet = setPredecessorAckField(
								oldRequest, extendedFields, systemId,
								ruleResult, fieldHierarchyHT, 
								TataPowerUtils.HOC_INDEX);
						if (isPredecessorAckDateFieldSet)
							return ruleResult;
					}					
					default: break;
	...
	static boolean setPredecessorAckField(Request oldRequest,
		...
		for (int i=(receiptDateIndex + 1); i<=TataPowerUtils.FINANCE_INDEX; i++){													
