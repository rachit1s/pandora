...
public class UserMapManager 
{
	 ...
	 */	
	public static final String TABLE_NAME = "ccr_user_map" ;
	 ...
	 */
	public static final String SUB_CORRES = "type" ; // varchar(50) not null
	/**
...
//										"	FROM " + TABLE_NAME + " join users on users.user_id=" + TABLE_NAME + "."  + USER_ID ;  
	...
	public static Hashtable<String,Hashtable<String, Hashtable<Integer,ArrayList<String>>>> getCompleteTableAsUserLogin( String firm ) throws TBitsException
	{
		// steps
		...
		try
		{
			...
			while(rs.next())
			{
				...
				String ul = rs.getString("user_login") ;
				uil.put(uid,ul) ;		
			}
			ps.close() ;
			
			PreparedStatement ps1 = con.prepareStatement(GET_UNIQUE_FIRM_USERS) ;
			ps1.setString(1, firm) ;
			ResultSet rs1 = ps1.executeQuery() ; 
					 
			while( rs1.next() )
			{
				// get first user and fill the table for this user
				int muid = rs1.getInt(USER_ID) ;
				Hashtable<String, Hashtable<Integer,ArrayList<String>>> st = getCompleteMappingAsUserLogin(muid) ;
				String userl = uil.get(muid) ;
				ct.put(userl, st) ;
			}
		}
		catch( Exception e ) 
	...
	{
		Connection con = null ;
		System.out.println("Creating the table with sql query : " + CREATE_TABLE );
//		System.out.println("Droping the table with query : " + DROP_TABLE );
		try 
		{
			con = DataSourcePool.getConnection() ;
//			PreparedStatement ps = con.prepareStatement(DROP_TABLE) ;
			...
			ps.execute() ;
			System.out.println("Created the table : " + TABLE_NAME );
//			System.out.println("Droped the table : " + TABLE_NAME );
		}
		catch(Exception e)
		{
			e.printStackTrace() ;
...
//			Gson gson = new Gson() ;
//			String mapJson = gson.toJson(getCompleteTableAsUserLogin("WPCL")) ;
