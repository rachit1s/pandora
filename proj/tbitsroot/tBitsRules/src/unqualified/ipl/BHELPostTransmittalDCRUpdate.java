 ...
 *
 */
public class BHELPostTransmittalDCRUpdate implements ITransmittalRule {
		...
		if ((businessAreaType == IPLUtils.DCR_BUSINESS_AREA) && 
			...
			if (transmittalType.equals("TransmitToIPLE")){				
				...
				int slideOffset = IPLUtils.getExFieldSlideOffset(connection, currentBA.getSystemId(), dcrRequest, paramTable,
												12, 9, isAddRequest, IPLUtils.DTN_TO_IPLE_SITE);				
				if (slideOffset != 0){
					Timestamp gmtDueDate = IPLUtils.getSlidedDueDate(isAddRequest, slideOffset);
					paramTable.put(IPLE_RESPONSE_DATE, gmtDueDate.toCustomFormat("yyyy-MM-dd HH:mm:ss"));
				}					
