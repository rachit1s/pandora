...
import static idc.com.tbitsGlobal.server.IDCUtils.IDC_COMMENTS_COMPLETE;
...
public class IDCPreRule implements IRule {
	...
	public RuleResult execute(Connection connection, BusinessArea ba,
		...
		try {
			commentsComplete = (Boolean)currentRequest.getObject(IDC_COMMENTS_COMPLETE);
			if(commentsComplete)
			{
				...
				while(i.hasNext()){
                   ...
                   iterList.addAll(assList);
					for(RequestUser reqUsr :iterList){
						if(reqUsr.getUserId()==currentUser.getUserId())
						{
