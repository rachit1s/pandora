...
var ttFontFace     = "tahoma, arial, verdana, trebuchet ms, sans-serif";
var ttFontSize     = "12px";
...
function tt_Hide()
{	if(window.tt_obj)
	{		if(window.tt_rdl) window.clearTimeout(tt_rdl);		if(!tt_sticky || !tt_act || notInToolTip)
		{
			 ...
			 * remove the div.
			 */
            var myobj = document.getElementById('tOoLtIp00');
            if (myobj && myobj.parentNode)
            {
...
function tt_Htm_new(tt, t_id, txt)
{
	var t_bgc = (typeof tt.T_BGCOLOR != tt_u)? tt.T_BGCOLOR : ttBgColor,
	t_bgimg   = (typeof tt.T_BGIMG != tt_u)? tt.T_BGIMG : ttBgImg,
	t_bc      = (typeof tt.T_BORDERCOLOR != tt_u)? tt.T_BORDERCOLOR : ttBorderColor,
...
// **** change
                                 t_tj.originalfunc = t_tj.onmouseover;
// **** change
//####
				t_tj.onmouseover = 
                                        new Function('e',
                                        ' var txt ='+
					'(unescape(this.originalfunc())).wzReplace("& ","&");'+
                                         'addDiv(this,"tOoLtIp00",txt);'+
                                       	'tt_Show(e,'+
					'"tOoLtIp' +0+''+0+ '",'+
					...
					');tt_Hide();addDiv(this,"tOoLtIp00",txt);'+
                                       	'tt_Show(e,'+
					'"tOoLtIp' +0+''+0+ '",'+
					(typeof t_tj.T_ABOVE != tt_u)+','+
					((typeof t_tj.T_DELAY != tt_u)? t_tj.T_DELAY : ttDelay)+','+
...
');'
                ...
                if ((typeof t_tj.T_HIDE_ONMOUSEOUT != tt_u))
                {
	...
	} // **** change document.write removed
//####
    var newdiv = document.createElement('div'); 
    newdiv.id = "verycomplexid";
    document.body.appendChild(newdiv);
var permframe = document.createElement('iframe'); 
permframe.id = "TTiEiFrM";
document.body.appendChild(permframe);
permframe.style.position="absolute";
permframe.style.display="none";
permframe.setAttribute("scrolling","no");
permframe.setAttribute("frameborder","0");
permframe.setAttribute("src","about:blank");
}

// **** change: function added
function addDiv(object,idname,text)
{	if (text != "" && notInToolTip != true)	{
		var htm = tt_Htm(object,idname,text);
		document.getElementById('verycomplexid').innerHTML = htm;	}	
}
