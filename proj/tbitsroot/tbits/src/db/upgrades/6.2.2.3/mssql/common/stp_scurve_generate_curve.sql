...
ALTER PROCEDURE [dbo].[stp_scurve_generate_curve] 
	...
	@curve_id int = 0,
	@weightageField varchar(50) = 'Weightage',
	@factorField varchar(50) = 'factors',
	@earlydateField varchar(50) = 'early_start_datetime',
	@latedateField varchar(50) = 'late_start_datetime'
AS
BEGIN
	...
	into #revisions
	from
		(select ae.sys_id, ae.request_id, ae.action_id, cast(ae.varchar_value as real) as real_value
		from actions_ex ae 
		join scurve_curve_requests scr on ae.request_id=scr.request_id and scr.curve_id=@curve_id
		join fields f on f.name=@weightageField and ae.sys_id=f.sys_id and ae.field_id=f.field_id 
		where ae.varchar_value != '' and ae.varchar_value is not null 
		and ae.varchar_value LIKE '[0-9]%[.][0-9]%'
		or ae.varchar_value LIKE '[0-9]%'
		or ae.varchar_value LIKE '[.][0-9]%'
		) weights_table
	join
		...
		from actions_ex ae 
		join scurve_curve_requests scr on ae.request_id=scr.request_id and scr.curve_id=@curve_id
		join fields f on f.name=@factorField and ae.sys_id=f.sys_id and ae.field_id=f.field_id
	...
	select ae.request_id, cast(aex.varchar_value as real) as weightage, sf.factor, 
	...
	into #predictions
	from requests_ex ae
	join fields f on ae.sys_id=f.sys_id and ae.field_id=f.field_id and f.name=@earlydateField
	join requests_ex aex on ae.sys_id=aex.sys_id and ae.request_id=aex.request_id --and ae.action_id=aex.action_id
		and aex.varchar_value != '' and aex.varchar_value is not null
		and aex.varchar_value LIKE '[0-9]%[.][0-9]%'
		or aex.varchar_value LIKE '[0-9]%'
		or aex.varchar_value LIKE '[.][0-9]%'
	join fields f1 on aex.sys_id=f1.sys_id and aex.field_id=f1.field_id and f1.name=@weightageField
	join requests_ex aex1 on ae.sys_id=aex1.sys_id and ae.request_id=aex1.request_id --and ae.action_id=aex1.action_id
	join fields f2 on aex1.sys_id=f2.sys_id and aex1.field_id=f2.field_id and f2.name=@latedateField
	join scurve_factors sf on sf.curve_id=@curve_id
	join scurve_curve_requests scr on ae.request_id=scr.request_id
	where scr.curve_id=@curve_id

	declare @tri int
	declare @tw real
	declare fill_values cursor for
	select distinct request_id, weightage from #revisions --#predictions why is this taken from predictions
	open fill_values
