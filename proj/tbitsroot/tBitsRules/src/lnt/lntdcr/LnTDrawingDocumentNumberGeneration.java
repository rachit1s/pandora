	...
	public RuleResult execute(Connection connection, BusinessArea ba,Request oldRequest, Request currentRequest, int Source, User user,
		...
		if ((boolObj != null) && ((Boolean) boolObj).booleanValue()) {
			...
			if (documentType.equals(DRAWING)) {
				 ...
				 */
				//formatter = PropertiesHandler.getProperty(LNTDCR_DRAWING_NUMBER_FORMATTER);
				...
				//formatter	= PropertiesHandler.getProperty(LNTDCR_DOCUMENT_NUMBER_FORMATTER); 	
...
*/
			...
			for(int i=1;i<=argsArray.length;i++)
			{
				...
				formatter=formatter.concat("%"+i+"$s-");
				if(i==argsArray.length)
					formatter=formatter.concat("%"+i+"$s");
			...
			try {
				...
				while(true){
					if(runningNumber > 0){
						...
						if(isExtended && APIUtil.isExtendedFieldValueUnique(connection, "requests_ex", "varchar_value", finalGeneratedDocNumber, field,false,true, ba.getSystemId(), currentRequest.getRequestId())){
							currentRequest.setObject(drawingFieldName, finalGeneratedDocNumber); // LNT_STD_NO,
							break;
						}
						else if(!isExtended && APIUtil.isFixedFieldValueUnique(connection, "requests", "varchar_value", finalGeneratedDocNumber, field,false,true, ba.getSystemId(), currentRequest.getRequestId()))
						{
