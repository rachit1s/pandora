...
public class DDCGridWizardPage extends
		TransmittalAbstractWizardPage<LayoutContainer, HashMap<String, String>> {

	private DDCPanel panel;
	ArrayList<DocNumberFileTuple> ModelCarryingForNotFoundDocs;
	ArrayList<DocNumberFileTuple> PathsOfTheFoundData;
	final DDCWizard holcimWizard;
	final WizardData wizardData;

	protected DDCGridWizardPage(UIContext wizardContext,
			DDCWizard holcimWizard, WizardData dataObject) {
		super(wizardContext);
	...
	public void buildPage(WizardData data) {
		...
		panel = new DDCPanel(wizardContext);

		panel.getSingleGridContainer().removeAllModels();
		panel.getCommonGridContainer().removeAllModels();

		DDCGridWizardPage.this.getWidget().setScrollMode(Scroll.NONE);
		DDCGridWizardPage.this.getWidget().add(panel, new FitData());

		DDCConstants.dbService.processRepoFileId(
				DDCMultiUploader.repoFileId, ClientUtils.getCurrentBA()
	...
	public boolean funcToBeCalledOnBack() {
		...
		DDCConstants.dbService.processGridData(
				...
				new AsyncCallback<HashMap<String, Object>>() {
					...
					public void onSuccess(HashMap<String, Object> result) {
						...
						if (!verdict) {
							...
							DDCConstants.dbService
									.AddAndUpdateRequest(
											DDCPluginConstants.deliverableFieldID,
