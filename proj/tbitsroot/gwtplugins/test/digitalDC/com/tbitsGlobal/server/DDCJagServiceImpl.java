...
import digitalDC.com.tbitsGlobal.client.DDCAttachmentInfo;
import digitalDC.com.tbitsGlobal.client.DDCJagService;
import digitalDC.com.tbitsGlobal.client.DocNumberFileTuple;
import digitalDC.com.tbitsGlobal.shared.DDCPluginConstants;

public class DDCJagServiceImpl extends TbitsRemoteServiceServlet implements
		DDCJagService {
	...
	HashMap<String, String> fetchConfigData(int aSystemId) throws SQLException {
		...
		try {
			...
			PreparedStatement ps = connection
					.prepareStatement("select * from ddc_config where sys_id=?");
				...
				while (rs.next()) {
					...
					if (rs.getString("field_name").equalsIgnoreCase("regex")) {
						regex = rs.getString("field_value");
							...
							"trn_process_id")) {
						DDCPluginConstants.setTRN_PROCESS_ID(Integer
								.parseInt(rs.getString("field_value")));

					} else if (rs.getString("field_name").equalsIgnoreCase(
							"docNoField")) {
						DDCPluginConstants.setDocField(rs
								.getString("field_value"));

					} else if (rs.getString("field_name").equalsIgnoreCase(
							"deliverableFieldID")) {
						DDCPluginConstants.setDeliverableFieldID(Integer
								.parseInt(rs.getString("field_value")));
	...
	public ArrayList<TbitsModelData> fetchFieldsdata(int aSystemId)
		...
		try {
			connection = DataSourcePool.getConnection();
			PreparedStatement ps = connection
					.prepareStatement("select * from ddc_fields where sys_id=?");
			ps.setInt(1, aSystemId);
			ResultSet rs = ps.executeQuery();
			if (rs != null) {
				while (rs.next()) {
			...
			if (hm != null) {
				...
				for (String fileName : hm.keySet()) {
					...
					DocNumberFileTuple processedData = getFiles(fileName,
							FilePath, DDCPluginConstants.fields);
				...
				if (FileMetaData != null && FileMetaData.size() > 0) {
					...
					this.search(FileMetaData, sysId, DDCPluginConstants
							.getDocField());
	...
	public DocNumberFileTuple getFiles(String fileName, String filePath,
		...
		String regex = DDCJagServiceImpl.regex;
		System.out
				.println(regex
	...
	public DocNumberFileTuple getFilesAfterKnowingSubject(int sysid,
		...
		int size = DDCPluginConstants.fields.size();
		Matcher m1 = pattern.matcher(name1);
		if (m1.matches()) {
			...
			for (int i = 1; i <= size; i++) {
				try {
					String str = m1.group(i);
					String name = DDCPluginConstants.fields.get(i - 1).get(
							"name");
					temp.set(name, str);
				} catch (IndexOutOfBoundsException e) {
					e.printStackTrace();
...
//		AddRequest addRequest = new AddRequest();
		try {
			...
			for (DocNumberFileTuple data : docNumberFileTuples) {
				 ...
				 */
				String docno = data
						.get(DDCPluginConstants.DocumentFieldNameToBeUsed);
				if (docno != null) {
	...
	public void AddRequest(String docNumberFieldName, int deliverableFieldID,
		...
		HashMap<Integer, HashMap<String, Collection<DDCAttachmentInfo>>> attachmentInfo = new HashMap<Integer, HashMap<String, Collection<DDCAttachmentInfo>>>();
		...
		try {
			...
			for (DocNumberFileTuple eachModelForAddition : modelDataForAddition) {
				...
				for (TbitsModelData tmd : DDCPluginConstants.fields) {
				...
				if (eachModelForAddition.getSubject() != null
						&& (!eachModelForAddition.getSubject().trim()
								.equals("")))
					aParamTable.put(DDCPluginConstants.TITLE,
				...
				try {
					...
					if (!attachmentInfo.containsKey(r.getRequestId())) {
						attachmentInfo
								.put(
										r.getRequestId(),
										new HashMap<String, Collection<DDCAttachmentInfo>>());
					}

					Collection<AttachmentInfo> attahmentInfoOfRequest = (Collection<AttachmentInfo>) r
							.getObject(deliverableField);
					DDCAttachmentInfo tempHolcimAttInfo = new DDCAttachmentInfo();
					ArrayList<DDCAttachmentInfo> tempCollectionOfHolcimInfo = new ArrayList<DDCAttachmentInfo>();
	...
	private boolean updateRequest(String docNumberFieldName,
			...
			for (DocNumberFileTuple data : docNumberFileTuple) {
				if (data.getRequestID() != -1) {
					...
					if (data.getSubject() != null
						...
						aParamTable.put(DDCPluginConstants.TITLE, data
								.getSubject());
		...
		HashMap<Integer, HashMap<String, Collection<DDCAttachmentInfo>>> attachmentInfo1 = (HashMap<Integer, HashMap<String, Collection<DDCAttachmentInfo>>>) hm
				.get("attachmentInfo");
	...
	public HashMap<String, Object> processGridData(
		...
		String docName = DDCPluginConstants.DocumentFieldNameToBeUsed;
	...
	public HashMap<String, Object> AddAndUpdateRequest(int deliverableFieldID,
		...
		try {
			...
			this.AddRequest(DDCPluginConstants.DocumentFieldNameToBeUsed,
					deliverableFieldID, modelDataForAddition, CurrentUser,
					sysId, dataObject);

			this.updateRequest(DDCPluginConstants.DocumentFieldNameToBeUsed,
	...
	public HashMap<String, Object> createDtn(int txnId, UserClient user,
		...
		TBitsResourceManager tbitsResMgr = new TBitsResourceManager();
		HashMap<Integer, HashMap<String, Collection<DDCAttachmentInfo>>> DDCAttachmentInfo = new HashMap<Integer, HashMap<String, Collection<DDCAttachmentInfo>>>();
		...
		try {
			...
			DDCAttachmentInfo = (HashMap<Integer, HashMap<String, Collection<DDCAttachmentInfo>>>) dtnDataMap
					.get("attachmentInfo");

			for (Integer reqId : DDCAttachmentInfo.keySet()) {

				ActualattachmentInfo.put(reqId,
						new HashMap<String, Collection<AttachmentInfo>>());
				HashMap<String, Collection<DDCAttachmentInfo>> tempHashMap = DDCAttachmentInfo
						.get(reqId);
				...
				for (String eachDeliverableField : tempHashMap.keySet()) {
					...
					ArrayList<DDCAttachmentInfo> attInfo = (ArrayList<DDCAttachmentInfo>) DDCAttachmentInfo
							.get(reqId).get(eachDeliverableField);

					ArrayList<AttachmentInfo> FinalAttInfo = new ArrayList<AttachmentInfo>();

					for (DDCAttachmentInfo eachHolcimAttachment : attInfo) {
	...
	void searchWithSubjectKnown(
			...
			String docNumberFieldName) throws TbitsExceptionClient {
...
//		AddRequest addRequest = new AddRequest();
		try {
			...
			throw new TbitsExceptionClient(ex);
		} finally {
			try {
				if ((conn != null) && !conn.isClosed()) {
					conn.close();
	...
	public HashMap<String, Object> fetchConstants(int systemId)
		...
		try {
			DDCPluginConstants.fields = fetchFieldsdata(systemId);
			...
			Collections.sort(DDCPluginConstants.fields, cmp);

			HashMap<String, String> hm1 = fetchConfigData(systemId);

			hm.put("fields", DDCPluginConstants.fields);
			hm.put("config", hm1);
