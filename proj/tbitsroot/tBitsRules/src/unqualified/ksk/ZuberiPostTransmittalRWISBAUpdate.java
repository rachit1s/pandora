 ...
 *
 */
public class ZuberiPostTransmittalRWISBAUpdate implements ITransmittalRule {

	/* (non-Javadoc)
	 * @see ksk.ITransmittalRule#getName()
	 */
	public String getName() {
		return this.getClass().getSimpleName() + ": Updates the RWIS-CEC business area post ZUBERI transmittal.";
	}
	...
	public void process(Connection connection, int transmittalId, Request transmittalRequest,
		...
		if (KSKUtils.isZUBERIDCR(dcrBA) && currentBA.getSystemPrefix().equals(KSKUtils.RWIS_CEC)){
...
//			paramTable.put(KSKUtils.SYSTEM_DEFINITION, dcrRequest.get(KSKUtils.SYSTEM_DEFINITION));
			...
			if (transmittalType.equals(KSKUtils.TRANSMIT_TO_RWIS_PHO)){
				...
				int slideOffset = KSKUtils.getSlideOffset(connection, currentBA.getSystemId(), dcrRequest, paramTable, 7, 7, isAddRequest);				
				if (slideOffset != 0){
			...
			/*else if (transmittalType.equals(KSKUtils.TRANSMIT_FOR_AS_BUILT)){
				...
				//paramTable.put(Field.ASSIGNEE, "WPCL");
				paramTable.put(Field.DUE_DATE, "");
				paramTable.put(KSKUtils.FIELD_INCOMING_TRANSMITTAL_NO, dtnNumber);trnIdPrefix 
