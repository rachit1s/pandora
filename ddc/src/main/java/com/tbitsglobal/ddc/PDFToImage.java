...
//	private static final String RESOLUTION = "-resolution";
	 ...
	 */
	public static File getImageFile(String pdfFile)  throws Exception{
//		String password = "";
		
		String outputPrefix = pdfFile.substring(0, pdfFile.indexOf('.'));
...
//		}
			...
			try {
				...
				if ("bilevel".equalsIgnoreCase(color)) {
					...
					System.err
							.println("Error: the number of bits per pixel must be 1, 8 or 24.");
					System.exit(2);
				}
				...
				boolean success = imageWriter.writeImage(document, imageFormat,
						"", 1, 1, outputPrefix, imageType,
						resolution);
				if (!success) {
					System.err
							.println("Error: no writer found for image format '"
									+ imageFormat + "'");
					System.exit(1);
				}
			} catch (Exception e) {
			...
			File f = new File(outputPrefix.concat("1.").concat(imageFormat));
			return f;
		}
