...
public class SingleAssignee implements IRule{
	...
	/**
	 * @author sharan
	 * Ruel:- During submit correspondence error pops up as ”Please select only one Assignee ” or “Please select the appropriate Assignee for the corr protocol 
...
if(assigneeField!=null)
	{ 
		...
		{
			return new RuleResult(false,"Please select the appropriate Assignee for the Corr Protocol selected.", true);
...
return new RuleResult(false,"Assignee field is empty. Please select an assignee",true);
}
}
return new RuleResult();
//	}
