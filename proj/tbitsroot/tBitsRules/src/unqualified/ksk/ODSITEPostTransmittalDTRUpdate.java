 ...
 *
 */
public class ODSITEPostTransmittalDTRUpdate implements ITransmittalRule {

	/* (non-Javadoc)
	 * @see ksk.ITransmittalRule#getName()
	 */
	public String getName() {
		return this.getClass().getSimpleName() + ": Site initiated post transmittal update in DTR(IntDTR) BA";
	}
	...
	public void process(Connection connection, int transmittalId, Request transmittalRequest,
		...
		if(dcrBA.getSystemPrefix().equals("ODSITE") && currentBA.getSystemPrefix().trim().equals("ODDTR")){
			...
			//if (transmittalType.equals("TransmitToCEC")){
				//paramTable.put(Field.USER, "DC-SITE");
				paramTable.put(Field.DESCRIPTION, "Documents sent by SITE via DTN #" 
