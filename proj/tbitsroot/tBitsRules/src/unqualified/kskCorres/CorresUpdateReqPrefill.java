...
public class CorresUpdateReqPrefill implements IUpdateRequestFooterSlotFiller
{
	...
	static class UserMap
	{
		String userLogin ;
		String corr ;
		String corrDisplayName ;
		ArrayList<String> to ;
		ArrayList<String> cc;
		public UserMap( String ul, String cor, ArrayList<String> toa, ArrayList<String> cca, String cdn )
		{
			userLogin = ul ;
			corr = cor ; 
			to = toa ;
	...
	public static String getUserMapValue( String firm, String login_user, String boss, ArrayList<String> prevSubsArray, Hashtable<String,Hashtable<String, Hashtable<Integer,ArrayList<String>>>> map, int systemId ) throws TBitsException 
	{
//		Hashtable<String,Hashtable<String, Hashtable<Integer,ArrayList<String>>>> map = UserMapManager.getCompleteTableAsUserLogin(firm) ;
		...
		for( java.util.Enumeration<String> users = map.keys() ; users.hasMoreElements() ;  )
		{
			...
			for( java.util.Enumeration<String> cors = corTable.keys() ; cors.hasMoreElements() ; )
			{
				...
				Type corType = null ;
				try {
					corType = Type.lookupAllBySystemIdAndFieldNameAndTypeName(systemId, KskConstants.CORR_TYPE_FIELD_NAME, cor );
				...
				String corrDisplayName = "" ;
				if( null != corType )
					corrDisplayName = corType.getDisplayName() ;
				...
				nccs.remove(user) ;
				userMapArray.add( new UserMap(user,cor,toList,new ArrayList<String>(nccs), corrDisplayName) )  ;
			}
		}
...
//			tagTable.put( KskConstants.EXT_SUBMIT_BUTTON_LIST, disSub ) ;
			return disSub;
		}
			
		URL fileURL = getClass().getResource(KskConstants.UPDATE_PREFILL_FILE_NAME) ;
		if( null == fileURL )
		{
...
//		}	
	...
	//	params.put("logger_value", login_user.getUserLogin() ) ;				
		// get  user info	
		String login_firm = ( login_info.get(UserInfoManager.FIRM) != null ? login_info.get(UserInfoManager.FIRM).trim().toUpperCase() : "" ) ;	
//		boolean isWPCLMember = login_firm.trim().equalsIgnoreCase( KskConstants.WPCL_FIRM_NAME ) ? true : false ;
		
		if( !login_firm.equalsIgnoreCase(WPCL_FIRM_NAME) && !login_firm.equalsIgnoreCase(SEPCO_FIRM_NAME) )
			return disableSubmit("You are not allowed to create correspondence.");
		...
		try
		{
			String corrInitName = (String) myRequest.get(KskConstants.CORR_CORRESPONDANCE_INITIATOR_FIELD_NAME) ;
			Type corrInitType = Type.lookupBySystemIdAndFieldNameAndTypeName(ba.getSystemId(), KskConstants.CORR_CORRESPONDANCE_INITIATOR_FIELD_NAME , corrInitName ) ;
			...
			//initialTypeIndex_value
//			String corrTypeName = (String)myRequest.myMapFieldToObjects.get(KskConstants.CORR_TYPE_FIELD_NAME) ;
//			Type corrType = Type.lookupAllBySystemIdAndFieldNameAndTypeName(ba.getSystemId(), KskConstants.CORR_TYPE_FIELD_NAME,corrTypeName ) ;
//			String initialTypeIndex_value = Integer.toString(corrType.getOrdering() - 1 );
//			params.put("initialTypeIndex_value", initialTypeIndex_value) ;
			...
			if( login_firm.equalsIgnoreCase(SEPCO_FIRM_NAME) )
			{			
...
//				userMap_value = new Gson().toJson(uma) ;
				Hashtable<String,Hashtable<String, Hashtable<Integer,ArrayList<String>>>> map = UserMapManager.getCompleteTableAsFirm(KskConstants.SEPCO_FIRM_NAME) ;
				userMap_value = getUserMapValue(KskConstants.SEPCO_FIRM_NAME, login_user.getUserLogin() , KskConstants.KSK_BOSS_LOGIN, new ArrayList<String>() , map, ba.getSystemId() ) ;

			}
			else
			{
				Hashtable<String,Hashtable<String, Hashtable<Integer,ArrayList<String>>>> map = UserMapManager.getCompleteTableAsFirm(KskConstants.WPCL_FIRM_NAME) ;
//				map.putAll( UserMapManager.getCompleteTableAsUserLogin(KskConstants.SEPCO_FIRM_NAME) ) ;
				userMap_value = getUserMapValue(KskConstants.WPCL_FIRM_NAME, login_user.getUserLogin() , KskConstants.KSK_BOSS_LOGIN, new ArrayList<String>() , map , ba.getSystemId() ) ;
			}
