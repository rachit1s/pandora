...
public class HolcimColumnConfig {
	...
	public ArrayList<ColumnConfig> configureColumns(BulkGridMode gridMode) {
		...
		System.out.println(HolcimClientConstants.fields.size());
		System.out.println(HolcimClientConstants.DocumentFieldNameToBeUsed);
		if(HolcimClientConstants.DocumentFieldNameToBeUsed  == null){
			Window.alert("Data not loaded");
		}
		ColumnConfig name = new ColumnConfig(HolcimClientConstants.DocumentFieldNameToBeUsed, 200);
		name.setHeader(HolcimClientConstants.DocumentFieldNameToBeUsed);
		configs.add(name);
		...
		for(TbitsModelData tmd:HolcimClientConstants.fields){
			String field_name=tmd.get("name");
			if(field_name.equals(HolcimClientConstants.getDocField())){
				continue;
			}
