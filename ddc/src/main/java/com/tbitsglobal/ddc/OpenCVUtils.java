...
public class OpenCVUtils {
     ...
     * @return Loaded image
     */
    public static IplImage loadOrExit(File file,int flags) throws Exception{
        // Verify file
        if (!file.exists()) {
            throw new Exception("Image file does not exist: " + file.getAbsolutePath());
        }
        // Read input image
        IplImage image = cvLoadImage(file.getAbsolutePath(), flags);
        if (image == null) {
            throw new Exception("Couldn't load image: " + file.getAbsolutePath());
        }
        return image;
    }
