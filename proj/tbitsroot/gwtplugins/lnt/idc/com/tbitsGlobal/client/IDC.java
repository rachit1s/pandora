...
public class IDC implements EntryPoint, IWizardPlugin {
	...
	private static void allValidSrcBAs() {
		...
		idcService.getValidSrcBAs(new AsyncCallback<ArrayList<String>>() {
			...
			public void onSuccess(final ArrayList<String> result) {
				...
				if (result == null)
					Log
							.info("Valid Source Business Areas for IDC module Not Found Yet");
				validSrcBAs = result;
			...
			public void onFailure(Throwable caught) {
				Log
						.info(
								"Unable to find Valid Source Business Areas for IDC module",
								caught);
			}
		});
	...
	public AbstractWizard getWidget(ArrayList<Integer> param) {
		...
		String sysPrefix = ClientUtils.getCurrentBA().getSystemPrefix();
		idcService.getIDCRequest(param, sysPrefix,
				new AsyncCallback<RequestData>() {
					public void onFailure(Throwable caught) {
						// TODO Auto-generated method stubtype filter text
						caught.printStackTrace();
						System.out.println(caught.getMessage());
						Window
								.alert("Some Error Occurred While contacting the idcService");
						messageBox.close();
					}
	...
	public boolean shouldExecute(String sysPrefix) {
		...
		if (validSrcBAs == null||sysPrefix==null) {
			return false;

		}
		if (null != validSrcBAs && null != sysPrefix
				&& validSrcBAs.contains(sysPrefix))
			return true;
		else
			return false;
