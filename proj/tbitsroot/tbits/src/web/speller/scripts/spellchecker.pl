...
my $spellercss = '/web/speller/spellerStyle.css';
my $wordWindowSrc = '/web/speller/wordWindow.js';
...
sub printTextVar {
	for( my $i = 0; $i <= $#textinputs; $i++ ) {
		print "textinputs[$i] = unescape('$textinputs[$i]')\n";
	}
}
...
sub printCheckerResults {
	...
	# exec aspell command
	my $cmd = "$aspell_cmd $aspell_opts < $tmpfilename";
	my @cmdret = `$cmd` 
		or handleError( "System error: Aspell program execution failed (`$cmd`)" ) 
		and return;
