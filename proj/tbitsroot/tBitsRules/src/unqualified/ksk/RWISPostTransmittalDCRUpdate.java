 ...
 *
 */
public class RWISPostTransmittalDCRUpdate implements ITransmittalRule {

	/* (non-Javadoc)
	 * @see ksk.ITransmittalRule#getName()
	 */
	public String getName() {
		return this.getClass().getSimpleName() + ": Updates RWIS BA post transmittal.";
	}
	...
	public void process(Connection connection, int transmittalId, Request transmittalRequest,
		...
		if (KSKUtils.isRWISPHODCR(dcrBA) && (businessAreaType == KSKUtils.DCR_BUSINESS_AREA)){
			
			int dcrSystemId = dcrBA.getSystemId();
			String trnIdPrefix = KSKUtils.getTransmittalIdPrefix(connection, dcrSystemId, transmittalType);	
			
			if (transmittalType.equals(KSKUtils.TRANSMIT_TO_RWIS_CEC)){
				...
				if (existingUser != null)
					...
					paramTable.put(Field.USER, "DC-RWIS");
				...
				paramTable.put (Field.DESCRIPTION,"Document released to RWIS_ERC via letter of Transmital number #" 
