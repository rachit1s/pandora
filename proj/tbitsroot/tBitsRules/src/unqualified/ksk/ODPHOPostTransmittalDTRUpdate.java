 ...
 *
 */
public class ODPHOPostTransmittalDTRUpdate implements ITransmittalRule {

	/* (non-Javadoc)
	 * @see ksk.ITransmittalRule#getName()
	 */
	public String getName() {
		return this.getClass().getSimpleName() + "PHO post transmittal DTR update.";
	}
	...
	public void process(Connection connection, int transmittalId, Request transmittalRequest,
		...
		if (KSKUtils.isODPHODCR(dcrBA) && currentBA.getSystemPrefix().trim().equals("ODDTR")){
			...
			if (transmittalType.equals(KSKUtils.TRANSMIT_TO_SEPCO)){
				...
				//paramTable.put(Field.STATUS, KSKUtils.STATUS_DOCUMENT_COMMENTED_BY_PHO);
