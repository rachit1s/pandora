...
/*
 * 
 * This class inserts all the users which are assigned a particular role in the concerned 
 * BA according to the condition, in the usertype field of that condition.
...
public class InsertUsersIntoUserTypeFieldFromRole implements IRule {
	...
	private void executeCondition(int systemId, Request currentRequest,
		...
		//Checking for the condition field value
		if ((conditionField != null) && (utfuCondition.conditionFieldValue.trim().equals(currentRequest.get(conditionField.getName()))))
		{
			
			Field userTypeField = Field.lookupBySystemIdAndFieldId(systemId, utfuCondition.userTypeFieldId);
			//Get all the user which are assigned a particular role in a particular BA.
