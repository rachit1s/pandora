...
public class GenericTransmittalCreator {
	...
	private Request logRequestInTransmittalBA(
		...
		if (allAttachments != null) {
			...
			//Collection<AttachmentInfo> supp=null;
			int process=trnProcess.getTrnProcessId();
			try {
				PreparedStatement ps=connection.prepareStatement("select src_field_id,target_field_id from trn_dtn_attachment_mapping where trn_process_id=?");
				ps.setInt(1, process);
				...
				String fname=Field.lookupBySystemIdAndFieldId(trnProcess.getSystemId(), field_id).getName();
				int tfield_i=rs.getInt(2);
				String tfname=Field.lookupBySystemIdAndFieldId(trnProcess.getDtnSysId(), tfield_i).getName();
			Collection<AttachmentInfo> abc=	getAllAttachmentsByFieldWise(fname,allAttachments);
				if(!abc.isEmpty())
				addReqParamTable.put(tfname, AttachmentInfo.toJson(abc));
...
//krishna
		//	Collection<AttachmentInfo> supp=getAllSupportingattachments(connection,trnProcess,allAttachments);
			...
			if (!exTempList.isEmpty() && i==-1)
				addReqParamTable.put(dtnDeliverableField.getName(), AttachmentInfo
						.toJson(exTempList));
		}
...
//		}
	...
	private Request logRequestInApprovalBA(
		...
		if (allAttachments != null) {
			...
			Collection<AttachmentInfo> abc=	getAllAttachmentsByFieldWise(fname,allAttachments);
				if(!abc.isEmpty())
				aParamTable.put(tfname, AttachmentInfo.toJson(abc));
...
//krishna
	...
	private Collection<AttachmentInfo> getAllAttachmentsByFieldWise(
			String fname,
			HashMap<Integer, HashMap<String, Collection<AttachmentInfo>>> allAttachments) {
