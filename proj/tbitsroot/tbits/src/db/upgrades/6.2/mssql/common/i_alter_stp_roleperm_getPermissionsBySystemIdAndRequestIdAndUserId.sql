...
ALTER PROCEDURE [dbo].[stp_roleperm_getPermissionsBySystemIdAndRequestIdAndUserId]
(
	@systemId		int,
	@requestId 	int,
	@userId		int
...
FROM
	...
	FROM
		...
		JOIN roles r
                ...
                ON ru.sys_id = @systemId AND ru.request_id = @requestId AND ru.field_id = r.field_id
	WHERE
		rp.sys_id = @systemId AND
		(
			ru.user_id = @userId OR
