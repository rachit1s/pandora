...
BEGIN
{
  ...
  $postmaster        = '';
  @referers          = qw(transbittech.com alpha0 localhost);
  ...
  our @ignore_names = qw(submit);
  our @non_uic_names = qw(message);
  our $useauth = 1; # 0 if server doesnt use authentication. 1 if server does user authentication. 
  #If server uses authentication supply login/password
  our $user = "tbits"; #smtp login name
  our $pass = "tbits"; #smtp password
  ...
  if(our $useauth)
	{
		...
		my $user = encode_base64(our $user);
		$self->_smtp_command($user);
		my $pass = encode_base64(our $pass);
		$self->_smtp_command($pass);
	}
...
Encrypts a string to the base 64. It is not an encryption. It is just translation.

Stolen from 
package MIME::Base64::Perl;
URL: http://search.cpan.org/~gaas/MIME-Base64-Perl-1.00/lib/MIME/Base64/Perl.pm
