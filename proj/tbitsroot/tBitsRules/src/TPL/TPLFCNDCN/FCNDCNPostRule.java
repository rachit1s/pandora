...
public class FCNDCNPostRule implements IPostRule {
	...
	public static String SI_SUFFIX="_SI";
		...
		if(ba!=null){
			...
			if(baPrefix.endsWith(FCN_SUFFIX)){
				...
				if(baPrefix.endsWith(DCN_SUFFIX)){
					...
					if(baPrefix.endsWith(SI_SUFFIX)){
						String description="SI Created via  ["+ba.getSystemPrefix()+"#"+currentRequest.getRequestId()+"]";
						updateSourceRequests(currentRequest,description);
					
					}
					else
						if(baPrefix.endsWith(TQ_SUFFIX)){
							String description="Tech Query  Created via  ["+ba.getSystemPrefix()+"#"+currentRequest.getRequestId()+"]";
							updateSourceRequests(currentRequest,description);
						}
